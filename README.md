...
As receitas e a lista de paises clicaveis sao carregadas diretamente do CSV local `assets/paises-mapa.csv`. O GeoJSON continua sendo usado para desenhar o mapa-mundi inteiro no globo.
...
`assets/paises-mapa.csv` contem os dados exibidos no modal: pais, nome da receita, imagem, descricao e link da materia.
...
3. A funcao `loadRecipesCsvText()` carrega `assets/paises-mapa.csv`.
...
As receitas ficam em `assets/paises-mapa.csv`.
...
Para adicionar, remover ou atualizar paises clicaveis e exibidos na busca, edite `assets/paises-mapa.csv`.
